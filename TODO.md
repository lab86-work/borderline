...
- [x] Compare providers on cost (monthly), bandwidth caps, and HD-streaming feasibility
- [x] **Decision**: pick one provider for the initial implementation → **AWS (sa-east-1, t4g.nano)**
...
- [x] Design Terraform module layout (provider agnostic + provider-specific)
- [x] Write basic Terraform config for chosen provider (VPC, firewall, VM, keypair)
- [x] Write cloud-init / Ansible / provisioner to install VPN
- [x] Automate credential generation for clients (QR code / config file)
